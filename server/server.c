...
int main() {
  ...
  while (1) {
    ...
    if (pid == 0) {
      ...
      // validamos comandos que no retornan salida
      ...
      if (bytes_read > 0) {
        ...
        // Se ejecuto un comando que no retorna salida (e.g. mkdir)
        if (send(client_socket, "Comando ejecutado con éxito",
                 strlen("Comando ejecutado con éxito"), 0) == -1) {
